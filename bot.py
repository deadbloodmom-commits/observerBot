...
async def adm_broadcast_cancel(call: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text("❌ Отменено.", reply_markup=get_admin_panel_kb())
...
async def adm_broadcast_execute(call: types.CallbackQuery, state: FSMContext):
    ...
    for u in users:
        try:
            ...
            with get_db_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("UPDATE users SET blocked = 1 WHERE user_id = %s", (u[0],))
                    conn.commit()
            blocked += 1
...
async def main():
    ...
    # Одновременный запуск веб-сервера для Render и polling для бота

...
import asyncpg
import psycopg2  # <--- Используем стандартный драйвер для psycopg2-binary[cite: 7]
...
# Загружает переменные из локального файла .env (если он есть)[cite: 7]
load_dotenv()

# Получаем токен из системных переменных хостинга или .env (используем BOT_TOKEN)[cite: 7]
TOKEN = os.getenv("BOT_TOKEN")

if not TOKEN:
    raise ValueError(f"Токен не найден! Доступные ключи: {list(os.environ.keys())}")[cite: 7]

# Создаем бота и диспетчер ОДИН раз[cite: 7]
bot = Bot(token=TOKEN)
...
# Кэш сообщений для синхронизации ответов (reply) и реакций[cite: 7]
msg_map = {}
# --- FSM СОСТОЯНИЯ ---
...
def get_db_connection():
    return psycopg2.connect(DATABASE_URL)[cite: 7]
...
def get_setting(key: str, default: str = "") -> str:
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            row = cursor.fetchone()
    return row[0] if row else default[cite: 7]
...
async def cmd_ban_user_in_group(message: types.Message):
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT admin_tag FROM admins WHERE user_id = %s", (message.from_user.id,))
            admin_row = cursor.fetchone()[cite: 7]
...
async def adm_broadcast_start(call: types.CallbackQuery, state: FSMContext):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            count = cursor.fetchone()[0][cite: 7]
...
async def main():
    ...
    # Инициализация базы данных и команд[cite: 7]
    init_db()
    await set_bot_commands(bot)
    
    # Фоновая задача[cite: 7]
    asyncio.create_task(check_streaks_daily_task())
    
    # Сбрасываем старый вебхук, чтобы polling работал корректно
    await bot.delete_webhook(drop_pending_updates=True)
    
    print("🥀 Бот успешно запущен!")[cite: 7]
    
    # Запуск polling для бота[cite: 7]
